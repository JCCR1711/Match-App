...
const BusinessTabBar = ({
  ...
  const routes = state.routes.filter((route) =>
    BUSINESS_ROUTES.includes(route.name as (typeof BUSINESS_ROUTES)[number]),
  );
    ...
    <View style={[styles.wrapper, { paddingBottom: Math.max(insets.bottom, 10) }]}>
      ...
      >
        {routes.map((route) => {
          ...
          const TabIcon = ICONS[route.name as keyof typeof ICONS];
          ...
          return (
            ...
            >
              <CustomIcon
                icon={TabIcon}
                color={focused ? theme.colors.white : theme.colors.authTextSecondary}
                sizeToken="large"
